...
# sys.path.insert(0,".")  # Prioritize local imports
sys.path.append("../../")
...
def construct_job(msize, bsize, csize, ch):
    ...
    if bsize != 0:
        # Blocked
        outdir = os.path.join(out_root, f"{msize}_{bsize}/{csize}_{ch}")
        args = [
            f"--l1d_size={csize}",
            f"--l1d_assoc=8",
            str(ch),
        ...
        # Base
        outdir = os.path.join(out_root, f"{msize}_base/{csize}_{ch}")
        args = [
            f"--l1d_size={csize}",
            f"--l1d_assoc=8",
            str(ch),
        ...
        for bsize in block_sizes[msize]:
            for csize in cache_sizes:
                for ch in num_channels:
                    job = construct_job(msize, bsize, csize, ch)
                    outdir = job[0]
