...
for x_param in x_param_keys:
    ...
    longer_param, shorter_param = sorted(
        other_params, key=lambda p: len(all_params[p]), reverse=True
    )

    color_map = {
        v: colors[i % len(colors)]
        for i, v in enumerate(all_params[longer_param])
    }
    style_map = {
        v: line_styles[i % len(line_styles)]
        for i, v in enumerate(all_params[shorter_param])
    }
                ...
                for long_val in all_params[longer_param]:
                    for short_val in all_params[shorter_param]:
                        ...
                        for x_val in x_values:
                            ...
                            if block is None:
                                out_dir = (
                                    f"out/{msize}_base/{csize}_{assoc}_{rp}"
                                )
                            else:
                                out_dir = (
                                    f"out/{msize}_{block}/{csize}_{assoc}_{rp}"
                                )
