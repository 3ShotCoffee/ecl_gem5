...
block_styles = {"base": "-", 16: "--", 32: "-.", 64: ":", 128: (0, (1, 10)), 256: (0, (5, 10))}
...
def load_stats(path):
    ...
    with open(path, 'r') as f:
        for line in f:
            if "End" in line or line.strip().startswith("#") or not line.strip():
                continue
...
axis_params = {"assoc": assocs, "csize": cache_sizes, "rp": replacement_policies}
            ...
            for stat_label in stat_labels:
                ...
                for matrix_size in matrix_sizes:
                    ...
                    for x in axis_values:
                        assoc = x if axis_param == "assoc" else other1 if other_keys[0] == "assoc" else other2 if other_keys[1] == "assoc" else None
                        csize = x if axis_param == "csize" else other1 if other_keys[0] == "csize" else other2 if other_keys[1] == "csize" else None
                        rp = x if axis_param == "rp" else other1 if other_keys[0] == "rp" else other2 if other_keys[1] == "rp" else None
                    ...
                    if x_vals:
                        plt.plot(x_vals, y_vals, label=f"{matrix_size}-base",
                    ...
                    while bsize <= matrix_size:
                        ...
                        if x_vals:
                            plt.plot(x_vals, y_vals, label=f"{matrix_size}-{bsize}",
                                     color=matrix_colors[matrix_size],
                ...
                plt.tight_layout()
                out_file = os.path.join(plot_dir, f"{stat_label.replace('::', '_')}.png")
